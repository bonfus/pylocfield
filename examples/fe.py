...
# | # BCC Iron
# |
# | BCC Iron is an interesting puzzle for musr.
# | Here we compute the dipolar field field for
# | the tetrahedral sites.
# |

# | ## Step 1: define the sample
# |
# | We will first use the primitive cell.
# | This requires special care when defining the
# | Fourier components, which are always provided in
# | Cartesian space.
...
# | The ones reported below are appropriate for the primitive cell.
...
atoms_conv.info["mu"] = np.array(
    [
        [0.25, 0.5, 0.0],  # these are fractional coordinates
        # for the tetrahedral interstitial
...
# | ## Check the results
# |
# | Here you can check that the definition of the Fourier components
# | provided in the primitive and conventional cells are the same.a
# | This is because the lattice vectors are oriented in such a way
# | that the z direction is always along a second nearest neighbor bond.
...
# | ## Compute
# |
# | Same as above, but now we have 12 equivalent sites,
# | because we used the conventional simple cubic cell.
...
# Compute the same fields in real space
B_r = direct_sum(atoms_conv, r_c=200.0)
